...
**Group 2**, _Classic Agents_: This group will develop a set of agents with pre-defined behaviors that will be useful in the range of games that are targeted by the company.  Individual agents should be capable of one or more behaviors, with well-defined triggers to shift between them.  For example, a standard monster might use the "explore" behavior until it sees a player, then shift to the "chase" behavior.  If it's health drops low, it might shift to the "flee" behavior.  The group should implement either a large set of simpler behaviors (to control a variety of NPC types) or else a smaller number of more sophisticated behaviors.
