...
| Class               | Description |
| ------------------- | ----------- |
| **`BitVector`**     | Similar to <code>[std::bitset](https://en.cppreference.com/w/cpp/utility/bitset)</code>, but allows for size changing.  Ideally, std::vector<bool> should fill the role in the standard library, but it doesn't have the extra functionality for bitwise operations that would make having a BitVector so useful. |
